...
import { Plan, ProviderName, ProviderTheme } from '../types';
...
const AdminPage: React.FC<Props> = ({ theme }) => {
  const [plans, setPlans] = useState<Record<string, Plan[]>>(ALL_PLANS);
  ...
  const savePlans = (updatedPlans: Record<string, Plan[]>) => {
  ...
  const handleAddPlan = () => {
    ...
    const updatedPlans = {
      ...plans,
      [selectedProvider]: [...plans[selectedProvider], plan]
    };
  ...
  const handleDeletePlan = (planId: string) => {
    const updatedPlans = {
      ...plans,
      [selectedProvider]: plans[selectedProvider].filter(p => p.id !== planId)
    };
    savePlans(updatedPlans);
  };

  const handleEditPlan = (planId: string, updatedPlan: Partial<Plan>) => {
    const updatedPlans = {
      ...plans,
      [selectedProvider]: plans[selectedProvider].map(p => 
    ...
    <div className="min-h-screen bg-gray-50 pt-6">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex justify-center gap-2 mb-8">
          {(['JIO', 'AIRTEL', 'VI', 'BSNL'] as ProviderName[]).map(provider => (
            <button
              key={provider}
          ...
          {plans[selectedProvider]?.map((plan) => (
            ...
            >
              {editingPlan === plan.id ? (
                <EditPlanForm
                  plan={plan}
                  onSave={(updatedPlan) => handleEditPlan(plan.id, updatedPlan)}
