...
import { ProviderTheme, User, RechargeHistory } from '../types';
...
const HistoryPage: React.FC<Props> = ({ theme, user }) => {
  // Get user-specific history from localStorage
  const getHistory = (): RechargeHistory[] => {
    if (!user) return [];
    try {
      const stored = localStorage.getItem('rechargeHistory');
      const allHistory = stored ? JSON.parse(stored) : [];
      return allHistory.filter((h: RechargeHistory) => h.userId === user.email);
    } catch {
      return [];
    }
  };
  
  const [history, setHistory] = React.useState<RechargeHistory[]>(getHistory());
  
  // Refresh history when component mounts or user changes
  React.useEffect(() => {
