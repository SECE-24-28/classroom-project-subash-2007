...
const PlanCard: React.FC<Props> = ({ plan, theme, onRecharge }) => {
  return (
    <motion.div
    ...
    >
      ...
      {plan.category === 'POPULAR' && (
        <div className={`absolute top-0 right-0 px-3 py-1 bg-gradient-to-r ${theme.gradient} text-white text-xs font-bold rounded-bl-xl`}>
          BEST SELLER
        </div>
      )}
      ...
      <div>
        ...
        <div className="space-y-4 mb-6">
          <div className="flex items-center text-gray-600">
            ...
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-wider">Validity</p>
              <p className="font-semibold text-sm">{plan.validity}</p>
            </div>
          </div>
          ...
          <div className="flex items-center text-gray-600">
            ...
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-wider">Calls</p>
          ...
          <div className="mb-4">
             ...
             <div className="flex flex-wrap gap-2">
               {plan.ottBenefits.map((benefit, idx) => (
                 <span key={idx} className="px-2 py-1 bg-indigo-50 text-indigo-600 text-xs rounded-md font-medium">
                   {benefit}
                 </span>
               ))}
             </div>
